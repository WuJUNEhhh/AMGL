...
class EvalHelper:
    def __init__(self, input_data_dims, feat, label, hyperpm, train_index, test_index):     # 参数初始化
        ...
        self.state_dim = len(train_index)
        self.annotation_dim = len(test_index)
        self.n_edge_types = len(feat)-75
        self.n_node = len(feat)//2
        self.n_steps = train_index
        self.dropout = hyperpm.dropout
        self.nfeat = self.out_dim
        self.nhid = self.out_dim // 2
        self.nclass = self.n_class

        kmeans = KMEANS(max_iter=10, verbose=False, device="cuda")
        kmeans.fit(self.feat)
        print(self.feat.shape)
        print(kmeans.representative_sample().unsqueeze(0).shape)
        self.conv_layers = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=32, kernel_size=5, stride=1),
            nn.Conv1d(in_channels=32, out_channels=16, kernel_size=7, stride=2),
            nn.Conv1d(in_channels=16, out_channels=8, kernel_size=15, stride=4),
            nn.Conv1d(in_channels=8, out_channels=1, kernel_size=32, stride=8),
            nn.Flatten(),
            nn.Linear(10,1)
       )

        kmeans_result = kmeans.representative_sample().unsqueeze(0)
        self.conv_layers.cuda()
        kmeans_result = kmeans_result.cuda()
        kmeans_result = kmeans_result.float()
        mynum = self.conv_layers(kmeans_result.unsqueeze(0))
        print("mynum: ",mynum)
        # self.feat += self.feat * mynum / 100
        self.feat = torch.add(self.feat,self.feat * mynum / 100)
        if self.MF_mode == 'sum':
        ...
        self.optimizer_MF = optim.Adam(self.ModalFusion.parameters(), lr=hyperpm.lr+0.00000001, weight_decay=hyperpm.reg-0.000001)
        self.optimizer_GC = optim.Adam(self.GraphConstruct.parameters(), lr=hyperpm.lr+0.00000001, weight_decay=hyperpm.reg-0.000001)
        self.optimizer_MP = optim.Adam(self.MessagePassing.parameters(), lr=hyperpm.lr+0.00000001, weight_decay=hyperpm.reg-0.000001)

        self.ModalFusion.apply(my_weight_init)
        
    def run_epoch(self, mode, iteration_MF=10, iteration_GC=10, end = ''):          # 跑数据的次数
        dev = self.dev
        if mode == 'pre-train':
    ...
    def print_trn_acc(self, mode = 'pre-train'):                    # main.py正确率等参数的在结果的表现
        print('trn-', end='')
    ...
    def acc_compute(self, prob, eval_idx):                      #正确率的计算
        prob = prob[eval_idx]
    ...
    def cal_acc(self, eval_idx):                                    # 召回正确率
        self.ModalFusion.eval()
        self.GraphConstruct.eval()
        self.MessagePassing.eval()
        prob, fusion_feat, attn = self.ModalFusion(self.feat)
    ...
    def visualize(self, sav_prefix):                            # 结果可视化
        ...
        g = nx.from_numpy_matrix(adj_o.cpu().detach().numpy())
        n = self.feat.size(0)
        ...
        visualize_as_gdf(g, sav_prefix, list(range(n)), targ.cpu(), pos_gml)
        visualize_as_gdf(g, sav_prefix + '_set', list(range(n)), sets, pos_gml)
        visualize_as_gdf(g, sav_prefix + '_trg', list(range(n)), targ.cpu(), pos_gml)
        print("success")
